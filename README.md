...
覆盖 Pump.fun 内盘 / PumpSwap AMM / Four.meme 内盘 / Flap Bonding / PancakeSwap 外盘，自动识别交易路径，无需手动切换。小费自愿，默认 0。

## ✦ 2026-03 更新

- **BSC 路由升级到 FreedomRouter v6** — Four.meme + Flap + PancakeSwap 统一入口，Proxy 地址升级为 [`0x444444444444147c48E01D3669260E33d8b33c93`](https://bscscan.com/address/0x444444444444147c48E01D3669260E33d8b33c93)
- **Flap 协议接入** — 自动识别 Bonding / DEX 阶段，按代币状态切换 Portal 或 PancakeSwap 路径
- **统一报价与授权目标** — 插件改用合约返回的 `quoteBuy` / `quoteSell` 与 `approveTarget`
- **安全性增强** — 私钥不上传服务器，只在本地插件后台解密和使用；BSC 交易统一带 `deadline`，Proxy / Impl 已完成 BscScan 验证
...
| 目录 | 说明 |
|------|------|
| [FreedomRouter/](FreedomRouter/) | BSC 聚合路由合约 — 自动判断 Four.meme / Flap / PancakeSwap 路径 |
...
- **私钥不上传服务器** — 私钥只在本地浏览器插件后台使用，不依赖中心化后端托管
- **前端页面拿不到明文** — 私钥明文不会暴露给普通 UI 页面，只在本地后台内存中短暂解密
- **私钥本地加密** — 插件使用 `PBKDF2 + AES-256-GCM` 加密私钥，密钥不直接存储
- **自动锁定** — 支持超时自动锁定，减少长期解锁风险
- **链上时效保护** — BSC 侧 `buy` / `sell` 统一带 `deadline`
- **授权目标由合约返回** — 使用 `approveTarget` 避免错误授权
- **公开源码可审计** — FreedomRouter Proxy 与 Impl 已完成 BscScan 验证

### Solana

推荐 [GetBlock](https://getblock.io) — 注册即送免费额度
...
├── token.js          — 代币检测路由
│   ├── token-bsc.js  — BSC 代币（Four.meme / Flap / PancakeSwap）
